...
static __always_inline int pull_and_recast_data(struct __sk_buff *skb,
...
int filter_ingress(struct __sk_buff *skb)
{
	...
	if ((void *)(ipv6 + 1) > data_end) {
		if (pull_and_recast_data(skb, &data, &data_end,
	...
	//     if (pull_and_recast_data(skb, &data, &data_end, sizeof(struct ethhdr)
	//     + sizeof(struct ipv6hdr) + sizeof(struct srv6_hdr) + srv6_len))
	//         return TC_ACT_OK;
