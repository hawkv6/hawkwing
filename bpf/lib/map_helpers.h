...
static __always_inline int store_incoming_triple(struct xdp_md *ctx, struct ipv6hdr *ipv6, struct srh *srh)
{
    ...
    for (int i = 0; i < MAX_SEGMENTLIST_ENTRIES; i++) {
        ...
        if ((void *)srh + sizeof(struct srh) + sizeof(struct in6_addr) * (i + 1) + 1 > data_end) {
    ...
    if (srh->next_hdr == IPPROTO_TCP) 
    {
        ...
        if ((void *)tcp + sizeof(struct tcphdr) <= data_end) {  // Explicit bounds check
        ...
        if ((void *)udp + sizeof(struct udphdr) <= data_end) {  // Explicit bounds check
